...
> **Hermes가 프로젝트에 결합된 기존 Claude Code 하네스를 실제로 호출하고, 결과까지 대조하는 제어 스킬입니다.**
>
> 배포 버전 **4.3.0** · Linux/WSL · Hermes 오케스트레이터용 전체 폴더 패키지

## 해결하려는 문제 — 모델에게 묻는 것과 워커에게 맡기는 것은 다릅니다

이미 특정 프로젝트 폴더에서 Claude Code를 사용하고 있다고 가정해 봅시다. 그 워커는 프로젝트 지침을 읽고, 그곳의 스킬·도구·실행환경으로 일을 합니다. 여기서 **하네스**는 모델 이름이 아니라, 모델의 추론과 도구 실행을 연결하는 CLI 프로그램 및 실행 규칙을 뜻합니다.

Hermes에게 “저 프로젝트의 Claude Code에게 맡겨 줘”라고 했는데 Hermes가 직접 파일을 수정하거나 다른 에이전트를 만들었다면, 산출물이 맞아도 요청한 실행 경로는 지켜지지 않은 것입니다. 같은 모델을 선택하는 것만으로 기존 하네스의 지침·권한·환경이 이어지지도 않습니다.
...
<img src="./assets/orchestrator-worker.svg" width="100%" alt="사용자가 Hermes에 범위를 승인하면 Hermes가 카드와 인증을 확인하고 실제 Claude Code를 타깃 프로젝트에서 호출한다. 프로젝트 지침·도구·환경에 결합된 워커가 결과를 반환하고 Hermes가 독립 검증한다.">

- **오케스트레이터(Hermes)**: 작업 범위·난이도·예산을 정하고 호출 결과를 검수합니다. 이 제어 스킬은 여기에 등록합니다.
- **워커(Claude Code)**: 타깃 프로젝트의 지침과 기존 도구·환경으로 실제 작업을 수행합니다. 제어 스킬을 워커에 중복 설치하지 않습니다.
...
| 이 스킬 | 기존 타깃 프로젝트의 **외부 Claude Code 하네스 자체**를 `claude -p`로 호출하고 검증합니다. |
...
> **사용자**: 이 프로젝트의 Claude Code에게 승인된 작업 하나를 맡겨 줘. 다른 폴더는 건드리지 마.
>
...
준비물은 Hermes Agent, Python 3, Bash, `claude` CLI와 해당 워커의 인증 환경입니다. Hermes에는 `terminal`, `file` 도구가 필요합니다. 서버 HTTP 검증은 `curl`, 변경 대조는 Git 등 선택한 검증 절차의 도구도 준비합니다. 외부 패키지를 이 안내가 자동 설치하지는 않습니다.
...
(
  set -euo pipefail
  SOURCE="/absolute/path/to/claude-code-control"
  ...
  DEST="$DEST_HOME/skills/claude-code-control"
...
목록에서 `claude-code-control`이 나타나는지 확인합니다. 복사 도중 실패하면 성공으로 보지 말고 생성된 목적지를 조사하세요. 재실행으로 덮어쓰지 않습니다. 실행 중인 **같은 프로필** 세션에서 `/reload-skills` 후 `/skill claude-code-control`으로 로드하거나 새 세션을 시작합니다. 목록 등재는 등록 확인일 뿐 워커 연결 성공이 아닙니다.

GitHub 배포를 선택할 때 `owner/repo/skills/claude-code-control` 같은 식별자는 **형식 예시**이며 이 README가 원격 발행을 보증하지 않습니다. 실제 출처를 확인하고 `hermes --profile PROFILE skills inspect <실제 식별자>`로 검사하세요. HTTP/GitHub 다운로드는 명시적으로 참조된 지원 파일만 포함할 수 있으므로 README·assets·간접 참조까지 전부 설치된다고 가정하지 않습니다. 설치 후 전체 파일 목록과 manifest를 대조해야 합니다.
...
```text
오케스트레이터 스킬: claude-code-control 4.3.0
스킬 위치: <설치된 스킬 절대경로>
...
이 패키지에는 카드 생성 스키마나 타깃 자동 설치기가 없습니다. 위 내용은 **기존 계약을 조사하고 승인받는 운영 지시**입니다. `preflight_claude_auth.sh`는 [scripts/preflight_claude_auth.sh](scripts/preflight_claude_auth.sh)에 있습니다. Claude는 `HOME` 후보마다 `claude auth status --text`를 검사하고 `PRIMARY_READY_HOME`을 반환합니다. 이후 같은 HOME으로 호출합니다. CLI 종료코드만 보지 말고 JSON 내부 `is_error`, `errors`, `permission_denials`도 확인합니다.
...
`grade_recommended`는 워커 추천이고 `grade_assigned`는 Hermes의 최종 판단입니다. S는 정체성 확인, L0~L4는 단일 명령부터 전략·사람 판단 루프까지의 난이도 등급입니다. **배분 수치의 정본은 [turn-budget-grades.md](references/turn-budget-grades.md)**입니다. `--max-turns`로 한 호출의 turn 상한을 주고, 실행시간·로그 감시를 별도로 둡니다. 결과 JSON의 `num_turns`를 `max_turns_used`로 기록하며 값이 없으면 `null`입니다. `max_turns_allocated`와 비교해 소진 여부를 판단합니다. turn 수는 토큰이나 금액의 하드 한도가 아닙니다.
...
이것은 **난이도 판단 → 배분 → 실측 → 다음 판단 조정**의 운영 루프입니다. 자동 최적화 엔진이나 비용 절감률 보장이 아닙니다. 참조 문서의 Layer 2 자동 보정은 미구현 청사진입니다. Claude 예산표는 기존 사례를 출발점으로 삼지만 새 프로젝트의 성능 보장은 아닙니다.

### 3. 승인된 작업 하나를 실제 하네스로 호출합니다

`workdir=WS`에서 검증된 `HOME`으로 `claude -p`를 호출합니다. `--allowedTools`는 작업에 필요한 범위만 허용하고, `--max-turns`와 `--output-format json`을 지정합니다. 프롬프트에는 작업 범위, 금지 경로·행위, 필요한 입력 발췌, 완료 검증 기준을 넣습니다. L3/L4처럼 긴 작업은 background 실행과 진행 감시를 사용하고, **HITL(Human-in-the-Loop)**, 즉 사람 판단이 필요한 지점에서는 질문을 중계합니다. 권한 확대나 추가 설치를 사람이 모르게 진행하지 않습니다.

### 4. 회신과 산출물을 따로 검증합니다

[출력 parser](scripts/parse_claude_json.py)로 결과를 검사하고, 카드의 `skills[].verification`을 우선 실행합니다. 서버 검증 도구는 health와 models의 HTTP 상태를 검사합니다. host·port·경로는 카드와 config에서 해소하며, HTTP 성공만으로 업무 내용의 정확성까지 보증하지 않습니다.

**제어 경로(control-plane)**가 정상이라는 것과 **실제 결과(data-plane)**가 맞다는 것을 분리합니다. 결과 검증 전에는 `completed`로 보고하지 않습니다. [실행 원장](references/primary-fallback-policy.md)에 실제 호출 여부, 상태, 예산, 증거 경로, fallback 여부를 남깁니다. Hermes 직접 실행이나 `delegate_task`로 바꿨다면 “primary 실패 + fallback”이지 Claude Code 제어 성공이 아닙니다.
...
```bash
python3 -B scripts/test_claude_control_scripts.py
...
| [예산 규칙](references/turn-budget-grades.md) | 등급별 배분과 실측 기록 |
