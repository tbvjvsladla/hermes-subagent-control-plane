...
>
> 계기: 다단계 복합작업·결정론 배치를 단일 `codex exec` 에 떠넘겨 완수 전 timeout kill(claude 측 동형 사례 protocol_26061117). **codex 는 `--max-turns` 가 없다 — `timeout` 이 유일 budget**(`references/timeout-budget-grades.md`).
...
| **G4** | 복합 다단계 | Docker 기동 + DB 적재 + 평가 등 1프롬프트 다단계 | **bounded 단계 분할**: 각 단계를 별 호출로 쪼개고 Hermes 가 단계 사이 **체크포인트**(예: `/health`·`docker ps`·산출물 존재). 각 단계 timeout 은 **grade→예산표대로 충분히**(줄이지 않음) — codex 는 `--max-turns` 없이 `timeout` 이 유일 budget. 정본: `references/timeout-budget-grades.md`. | 단계별 `working` |

## 판단 기준 (요지)
- **"N건에 대해 동일 동작 반복" → CLI 위임 아님**(매 턴 LLM 추론 = turn 낭비). reasoning-집약(스크립트 설계·디버깅)만 위임.
- CLI 가 스크립트 작성·실행·디버깅을 **모두** 시도하면 timeout 내 미완료가 된다 — **작성과 실행을 분리**(CLI 설계 → Hermes 실행)하거나 단계 분할.
- 굳이 CLI 로 배치를 돌려야 하면 background + **충분한 timeout**(grade→예산표, 줄이지 않음) — codex 는 `--max-turns` 가 없다.
- **L4 원자화 예외**: 추론이 무거운 전략작업(전략+HITL)은 억지 원자화하지 않는다 — 원자화(`1위임=1작업`)는 *이질적 결정론 작업 결합*을 막는 규율이지 한 추론작업 내부를 쪼개라는 게 아니다(grade L4, `references/timeout-budget-grades.md`).
